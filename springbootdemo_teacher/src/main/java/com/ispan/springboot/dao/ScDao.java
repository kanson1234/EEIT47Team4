...
public interface ScDao extends JpaRepository<ShoopingCar, Integer> {
	...
	@Transactional
	@Modifying
	@Query(value = "DELETE FROM ShopingCar WHERE C1_Id = :cid ", nativeQuery = true)
	public void deleteByC1id(@Param("cid") Integer cid);
	...
	@Query(value = "SELECT SC_Count FROM ShopingCar WHERE C1_Id= :cid AND SH_Item_Id= :itid ", nativeQuery = true)
	public Integer check(@Param("cid") Integer cid, @Param("itid") Integer itid);
